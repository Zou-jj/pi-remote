...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    cv2.putText(frame, "AWB: r:{} b:{}".format(str(float(camera.awb_gains[0].__round__(2))),
                                               str(float(camera.awb_gains[1].__round__(2)))),
            ...
            if motionCounter >= conf["min_motion_frames"]:
                ...
                if dropUse:
                    ...
                    cv2.putText(outimg, "Shutter: 1/{}".format(str(round(1000000 / camera.exposure_speed))),
                                (outimg.shape[1] - 350, outimg.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    cv2.putText(outimg, "Meter: {}".format(str(camera.meter_mode)),
                                (outimg.shape[1] - 350, outimg.shape[0] - 40), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    cv2.putText(outimg, "Brightness: {}".format(str(camera.brightness)),
                                (outimg.shape[1] - 350, outimg.shape[0] - 70), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    cv2.putText(outimg, "ISO: {}".format(str(camera.iso)),
                                (outimg.shape[1] - 350, outimg.shape[0] - 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    cv2.putText(outimg, "Contrast: {}".format(str(camera.contrast)),
                                (outimg.shape[1] - 350, outimg.shape[0] - 130), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    cv2.putText(outimg, "AWB: r:{} b:{}".format(str(float(camera.awb_gains[0].__round__(2))),
                                                                str(float(camera.awb_gains[1].__round__(2)))),
                                (outimg.shape[1] - 350, outimg.shape[0] - 160), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    cv2.putText(outimg, "Framerate: {}".format(str(camera.framerate)),
                                (outimg.shape[1] - 350, outimg.shape[0] - 190), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 255, 0), 2)
                    if dropUse:
